...
public class ActivityClient {
	...
	public Activity get(String id) {
		WebTarget target = client.target("http://localhost:8080/fitbit-services/webapi/");
		Response response = target.path("activities/" + id).request(MediaType.APPLICATION_JSON).get(Response.class);
		
		if(response.getStatus() != 200) {
			throw new 
			RuntimeException(response.getStatus() + ": there was an error on the server!");
		}
	...
	public List<Activity> get() {
		WebTarget target = client.target("http://localhost:8080/fitbit-services/webapi/");
		// GenericType is given by Jersey to fix the issue of returning a Collection type, for the following we could not just have get(List.class) type!
